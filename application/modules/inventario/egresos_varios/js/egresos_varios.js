		...
		if(forma_valida){
			...
			var info = { 
				datosEncabezado:{
					...
					,id_categoria_operacion:"3"
					,oper:""
				}
				, datosDetalle : grid			
			};
			$.post(base_url+"inventario/devoluciones/grabarDevolucion",info,function( data ){
				respuesta = JSON.parse(data);
				if(respuesta["status"]== "error"){
					jAlert("Ha ocurrido un error" + respuesta["message"],'Informacion');				
				}else{
					jAlert('Se ha registrado la Devolución de forma exitosa','Informacion');
					location.reload();							
				}
